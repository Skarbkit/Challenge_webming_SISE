...
def detect_face_opti(img, age_model, gender_model, known_names, known_encodings):
    ...
    for (top, right, bottom, left) in faces:
        ...
        for i in range(len(known_encodings)):
            ...
            matches = face_recognition.compare_faces(known_encodings[i], faces, tolerance=0.7)
...
def video_capture(model_age, model_genre,know_names,known_encodings):
    # Charger les modèles
    model_age = model_age
    ...
    while True:
        ...
        if ret:
            ...
            #if count % 2 == 0:
            thread = threading.Thread(target=detect_face_opti, args=(frame,model_age,model_genre, know_names, known_encodings))
...
def main():
    ...
    video_capture(model_age, model_genre, known_names, known_encodings)
